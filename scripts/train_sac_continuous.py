    ...
    def step(
        ...
        info: dict[str, Any] = {
            ...
            "reward_components": {
                ...
                "lifting_object": np.zeros(self.num_envs, dtype=np.float32),
                "object_goal_tracking": np.zeros(self.num_envs, dtype=np.float32),
                "object_goal_tracking_fine_grained": np.zeros(self.num_envs, dtype=np.float32),
                "action_rate": action_penalty.astype(np.float32),
                "joint_vel": np.zeros(self.num_envs, dtype=np.float32),
            },
        }
